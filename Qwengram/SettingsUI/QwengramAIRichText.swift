...
private let qwengramInlinePattern = try! NSRegularExpression(pattern: #"\[([^\]]+)\]\((https?://[^\s)]+)\)|\*\*([^*\n]+)\*\*|__([^_\n]+)__|(?<!\*)\*([^*\n]+)\*(?!\*)|(?<!_)_([^_\n]+)_(?!_)|`([^`\n]+)`"#)

func qwengramRenderAIResponse(_ source: String, color: UIColor, linkColor: UIColor) -> NSAttributedString {
    ...
    for line in source.split(separator: "\n", omittingEmptySubsequences: false) {
        ...
        let matches = qwengramInlinePattern.matches(in: prefix, range: NSRange(location: 0, length: text.length))
        var cursor = 0
