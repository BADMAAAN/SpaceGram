...
These implementations are now owned within the Qwengram product layer. They
...
in the appropriate Qwengram module rather than expanding this compatibility layer.
...
need separate tests. The shared string loader was moved to `Qwengram/Strings`
and renamed to `QwengramStrings`; existing `ngI18n` calls and localization keys
remain supported by that single implementation.

The enhancement settings entry is displayed as **Qwengram · Telegram** alongside
the product's Qwengram settings. It retains the original navigation cases and
