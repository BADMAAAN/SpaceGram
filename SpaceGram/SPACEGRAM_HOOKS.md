...
- **File:** `submodules/Postbox/Sources/SeedConfiguration.swift`
  ...
  **Module:** Postbox API; no SpaceGram storage dependency.
...
- **File:** `submodules/TelegramCore/Sources/SyncCore/SyncCore_StandaloneAccountTransaction.swift`
  ...
  **Reason:** Installs `spaceGramBeforeMessageUpdate` for account Postbox writes.
  **Module:** TelegramCore / `SpaceGram/HistoryIntegration`.
...
- **File:** `submodules/TelegramCore/BUILD`
  ...
  **Reason:** Compiles `//SpaceGram/HistoryIntegration:Sources` inside TelegramCore
  and links `//SpaceGram/HistoryStorage:SpaceGramHistoryStorage`.
  Also links `//SpaceGram/Settings:SpaceGramSettings` for capture gating.
  **Module:** TelegramCore.
...
- **File:** `submodules/TelegramCore/Sources/State/AccountStateManagementUtils.swift`
  ...
  **Reason:** Marks only explicit server delete updates. During replay, calls
  `spaceGramBeforeServerDelete` before the existing physical deletion. Global IDs
...
The existing SpaceGram Settings Message History entry opens History Settings.
All three switches use `SpaceGramSettings` / `@SpaceGramDefault` with persistent
...
`SpaceGramSettingsSignal`. Each capture callback reads current settings before
snapshotting or accessing HistoryStorage: edit requires SpaceGram Enabled AND
Message History AND save-edited; server delete requires SpaceGram Enabled AND
...
The general SpaceGram Enabled preference also gates capture, without hiding
...
- `SpaceGram/SettingsUI` links `SpaceGram/HistoryUI` and opens its History browser
  from **History Settings > View History**, including while capture is disabled.
- `SpaceGram/HistoryUI` reads only `context.account.postbox` in a transaction on
...
- `SpaceGramHistoryStore.listRecords` is a read-only, per-entry fault-isolating
...
- **File:** `submodules/TelegramUI/Sources/ChatInterfaceStateContextMenus.swift`
  ...
  **Reason:** Joins the existing menu data signal with
  `spaceGramMessageHistoryAvailable`. Only a single non-service, non-scheduled
  ...
  **Navigation:** **Message History** dismisses the menu and pushes the public
  `spaceGramHistoryDetailController(context:messageId:)` entry point. It converts
...
- **File:** `submodules/TelegramUI/BUILD`
  ...
  **Reason:** Adds a direct dependency on `SpaceGramHistoryUI`. Its visibility now
...
- **File:** `submodules/SettingsUI/Sources/Privacy and Security/PasscodeOptionsController.swift`
  ...
  **Reason:** Lets SpaceGram Privacy & Security open Telegram's single passcode
...
- **File:** `submodules/TelegramUI/Components/TelegramAccountAuxiliaryMethods/Sources/TelegramAccountAuxiliaryMethods.swift`
  ...
  **Reason:** Reads the selected account's SpaceGram metadata policy off the main
...
- **File:** `submodules/TelegramUI/Components/PeerInfo/PeerInfoScreen/Sources/PeerInfoSettingsItems.swift`
  ...
  **Reason:** Adds the SpaceGram settings entry.
  **Module:** `SpaceGramSettingsUI`
...
- **File:** `submodules/TelegramUI/Components/PeerInfo/PeerInfoScreen/Sources/PeerInfoScreen.swift`
  ...
  **Reason:** Adds the SpaceGram settings destination.
  **Module:** `SpaceGramSettingsUI`
...
- **File:** `submodules/TelegramUI/Components/PeerInfo/PeerInfoScreen/Sources/PeerInfoScreenSettingsActions.swift`
  ...
  **Reason:** Routes the destination to `spaceGramSettingsController(context:)`.
  **Module:** `SpaceGramSettingsUI`
...
- **File:** `submodules/TelegramUI/Components/PeerInfo/PeerInfoScreen/BUILD`
  ...
  **Reason:** Links the SpaceGram settings UI module.
  **Module:** `SpaceGramSettingsUI`
  **Rebase note:** Preserve the direct Bazel dependency.

- **File:** `submodules/TelegramUI/Sources/ChatInterfaceStateContextMenus.swift`
  **Section:** `contextMenuForChatPresentationInterfaceState(...)`
  **Reason:** Adds the SpaceGram AI message context-menu entry for a single, non-empty, non-secret text message.
  **Module:** `SpaceGramSettingsUI`
  **Rebase note:** Preserve the local-only handoff to `spaceGramMessageAIController(context:text:)`; do not invoke an AI provider from the menu action.

- **File:** `submodules/TelegramUI/BUILD`
  **Section:** `TelegramUI` dependencies
  **Reason:** Makes `SpaceGramSettingsUI` available to the TelegramUI context-menu hook.
  **Module:** `SpaceGramSettingsUI`
  **Rebase note:** Preserve the direct Bazel dependency next to the other fork UI dependencies.

- **File:** `SpaceGram/Enhancements/Demo/Sources/NagramDemo.swift`
...
The effective policy lives in `SpaceGram/Settings/SpaceGramGhostPolicy.swift`.
All four Ghost preferences default to false and require SpaceGram Enabled.
Settings are device-wide like the existing SpaceGram preferences; each account's
...
with the SpaceGram policy for both visible-index handling and the live read-action
...
suppression off, or disabling SpaceGram, restores normal reading of visible messages.
...
- `TelegramCore/BUILD`: adds SpaceGramMediaArchive. HistoryIntegration's existing
  Swift filegroup includes SpaceGramMediaIntegration.swift; do not turn it into a
  separate module importing TelegramCore (that would create a dependency cycle).
- `SpaceGram/HistoryUI/BUILD`: adds MediaArchive and SpaceGramStrings. The existing
...
- `Tests/AllTests/BUILD`: includes SpaceGramMediaArchiveTests alongside TgCallsTests.
...
continue to use SpaceGram where they are stable integration contracts.

## SpaceGram cleanup after branding

The project-role badges inherited from Nagram were removed from
`PeerInfoHeaderNode.swift` and `PeerInfoScreen.swift`, along with their private
ID directory/localization helper. Telegram premium, verified, scam/fake and
emoji-status UI remains. No storage or peer model was changed. The deleted
Swift source was included only through the PeerInfoScreen source glob; no
separate target depended on it. See SPACEGRAM_ARCHITECTURE_AUDIT.md.
