...
public struct SpaceGramArchivedAsset: Codable {
...
public enum SpaceGramArchivedAssetState {
    case available(SpaceGramArchivedAsset)
    case expired
    case missing
    case corrupt
}
...
public final class SpaceGramMediaCapture {
    ...
    deinit {
        SpaceGramMediaArchive.releaseCapture(size: self.size)
    }
}

public final class SpaceGramMediaPreview {
    public let url: URL
    fileprivate init(url: URL) { self.url = url }
    deinit {
        let directory = url.deletingLastPathComponent()
        SpaceGramMediaArchive.queue.async {
            try? FileManager.default.removeItem(at: directory)
            SpaceGramMediaArchive.activePreviews -= 1
            SpaceGramMediaArchive.activePreviewPaths.remove(directory.path)
        }
    }
}
...
public enum SpaceGramMediaArchive {
    ...
    fileprivate static let queue = DispatchQueue(label: "SpaceGram.MediaArchive", qos: .utility)
    ...
    public static func usage(root: URL, completion: @escaping (Result<SpaceGramMediaArchiveUsage, SpaceGramMediaArchiveError>) -> Void) {
        queue.async {
            do {
                let usage = try withLock(root: root) { () -> SpaceGramMediaArchiveUsage in
                    let policy = try readPolicy(root: root)
                    let assets = try maintain(root: root, policy: policy)
                    return SpaceGramMediaArchiveUsage(bytes: assets.reduce(0) { $0 + $1.bytes }, assetCount: assets.count, policy: policy)
                }
                completion(.success(usage))
            } catch { completion(.failure((error as? SpaceGramMediaArchiveError) ?? .storage)) }
        }
    }

    public static func setPolicy(root: URL, policy: SpaceGramMediaArchivePolicy, completion: @escaping (Bool) -> Void) {
        queue.async {
            do {
                guard policy.isValid else { throw SpaceGramMediaArchiveError.invalidData }
    ...
    public static func reconcile(root: URL, referencedIds: Set<String>, referencesComplete: Bool) {
        ...
        queue.async {
            ...
            } catch { NSLog("SpaceGramMediaArchive: orphan reconciliation failed") }
        }
    }
    ...
    public static func root(mediaBoxPath: String) -> URL {
        return URL(fileURLWithPath: mediaBoxPath).deletingLastPathComponent().appendingPathComponent("spacegram-media-v1", isDirectory: true)
    }
    ...
    public static func pinCompletedFile(path: String, fileName: String, kind: String, fileExtension: String) -> SpaceGramMediaCapture? {
        ...
        return SpaceGramMediaCapture(fd: fd, info: info, name: fileName, kind: kind, ext: safeExtension)
    }
    ...
    public static func store(root: URL, captures: [SpaceGramMediaCapture], completion: @escaping ([SpaceGramArchivedAsset]) -> Void) {
        queue.async {
            var saved: [SpaceGramArchivedAsset] = []
            do {
                try withLock(root: root) {
                    let policy = try readPolicy(root: root)
                    ...
                    for capture in captures {
                        do {
                            ...
                            NSLog("SpaceGramMediaArchive: asset copy failed; Telegram continues")
                        }
                    }
                ...
                NSLog("SpaceGramMediaArchive: storage unavailable; Telegram continues")
            }
            completion(saved)
        }
    }

    public static func list(root: URL, ids: [String], completion: @escaping ([SpaceGramArchivedAsset]) -> Void) {
        queue.async {
            let wanted = Set(ids)
                ...
                try withLock(root: root) {
                    ...
                    for id in Set(ids) where UUID(uuidString: id) != nil {
                        ...
                        guard let values = try? metadata.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey, .isSymbolicLinkKey]),
                              ...
                              let asset = try? JSONDecoder().decode(SpaceGramArchivedAsset.self, from: data),
    ...
    public static func states(root: URL, ids: [String], capturedAt: [String: Int64], completion: @escaping ([String: SpaceGramArchivedAssetState]) -> Void) {
        queue.async {
            var result: [String: SpaceGramArchivedAssetState] = [:]
            do {
                try withLock(root: root) {
                    let policy = try readPolicy(root: root)
                    let cutoff = policy.automaticCleanup ? Date().timeIntervalSince1970 - Double(policy.retentionDays) * 24 * 60 * 60 : -Double.infinity
    ...
    public static func remove(root: URL, ids: [String]) {
        queue.async {
            ...
            } catch { NSLog("SpaceGramMediaArchive: cleanup failed") }
        }
    }
    ...
    public static func clear(root: URL, completion: @escaping (Bool) -> Void) {
        queue.async {
            do {
                ...
                NSLog("SpaceGramMediaArchive: clear failed")
                completion(false)
            }
        }
    }
    ...
    public static func preview(root: URL, id: String, completion: @escaping (Result<SpaceGramMediaPreview, SpaceGramMediaArchiveError>) -> Void) {
        queue.async {
            do {
                guard activePreviews < 4 else { throw SpaceGramMediaArchiveError.capacity }
                let preview = try withLock(root: root) { () -> SpaceGramMediaPreview in
                    guard let asset = try maintain(root: root, policy: readPolicy(root: root)).first(where: { $0.id == id }) else { throw SpaceGramMediaArchiveError.unavailable }
                    try maintainPreviews(reserving: asset.bytes)
                    let source = try FileHandle(forReadingFrom: payload(root: root, asset: asset))
                    defer { try? source.close() }
                    let parent = FileManager.default.temporaryDirectory.appendingPathComponent("spacegram-preview-" + UUID().uuidString, isDirectory: true)
                    ...
                    do {
                        guard try copyAndHash(from: source, to: destination, size: asset.bytes) == asset.sha256 else { throw SpaceGramMediaArchiveError.invalidData }
                    } catch {
                    ...
                    return SpaceGramMediaPreview(url: destination)
                }
                completion(.success(preview))
            } catch {
                completion(.failure((error as? SpaceGramMediaArchiveError) ?? .storage))
            }
        }
    }
    ...
    private static func withLock<T>(root: URL, _ body: () throws -> T) throws -> T {
        ...
        } catch SpaceGramMigrationError.conflictingDirectories {
            throw SpaceGramMediaArchiveError.migrationConflict
        }
        ...
        guard fd >= 0 else { throw SpaceGramMediaArchiveError.storage }
        defer { _ = Darwin.close(fd) }
        guard flock(fd, LOCK_EX) == 0 else { throw SpaceGramMediaArchiveError.storage }
        defer { _ = flock(fd, LOCK_UN) }
        return try body()
    }

    private static func payload(root: URL, asset: SpaceGramArchivedAsset) -> URL {
        return root.appendingPathComponent(asset.id + ".data." + asset.fileExtension)
    }

    private static func erase(root: URL, asset: SpaceGramArchivedAsset) throws {
        let fm = FileManager.default
    ...
    private static func readPolicy(root: URL) throws -> SpaceGramMediaArchivePolicy {
        let file = root.appendingPathComponent("policy.json")
        ...
        guard values.isRegularFile == true, values.isSymbolicLink != true,
              (values.fileSize ?? Int.max) <= 8192,
              let policy = try? JSONDecoder().decode(SpaceGramMediaArchivePolicy.self, from: Data(contentsOf: file)), policy.isValid else {
            throw SpaceGramMediaArchiveError.invalidData
        }
        return policy
    }

    private static func maintain(root: URL, policy: SpaceGramMediaArchivePolicy, forceExpiry: Bool = false) throws -> [SpaceGramArchivedAsset] {
        let fm = FileManager.default
        ...
        for file in files where file.pathExtension == "json" && UUID(uuidString: file.deletingPathExtension().lastPathComponent) != nil {
            ...
            guard info.isRegularFile == true, info.isSymbolicLink != true, (info.fileSize ?? Int.max) <= 8192,
                  let data = try? Data(contentsOf: file), let asset = try? JSONDecoder().decode(SpaceGramArchivedAsset.self, from: data),
    ...
    public static func retainedAssets(_ candidates: [SpaceGramArchivedAsset], storageLimitBytes: Int64, maxCount: Int = SpaceGramMediaArchive.maxAssets) -> [SpaceGramArchivedAsset] {
    ...
    private static func maintainPreviews(reserving bytes: Int64) throws {
        ...
        var total: Int64 = 0
        for folder in folders where folder.lastPathComponent.hasPrefix("spacegram-preview-") || folder.lastPathComponent.hasPrefix("qwengram-preview-") {
        ...
        guard total <= maxBytes - bytes else { throw SpaceGramMediaArchiveError.capacity }
    }

    private static func copyAndHash(from input: FileHandle, to destination: URL, size: Int64) throws -> String {
        guard FileManager.default.createFile(atPath: destination.path, contents: nil, attributes: [.protectionKey: FileProtectionType.completeUntilFirstUserAuthentication]) else { throw SpaceGramMediaArchiveError.storage }
        let output = try FileHandle(forWritingTo: destination)
        defer { try? output.close() }
        var hash = SHA256()
        var remaining = size
        while remaining > 0 {
            guard let data = try input.read(upToCount: Int(min(remaining, 1024 * 1024))), !data.isEmpty else { throw SpaceGramMediaArchiveError.invalidData }
            try output.write(contentsOf: data)
            hash.update(data: data)
            remaining -= Int64(data.count)
        }
        guard (try input.read(upToCount: 1) ?? Data()).isEmpty else { throw SpaceGramMediaArchiveError.invalidData }
