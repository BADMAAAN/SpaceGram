...
private final class SpaceGramQwenAssistantArguments {
...
private enum SpaceGramQwenAssistantEntry: ItemListNodeEntry {
    case header(Int32, Int32, String)
    case message(Int32, Int32, SpaceGramAIMessage, Bool)
    ...
    static func == (lhs: SpaceGramQwenAssistantEntry, rhs: SpaceGramQwenAssistantEntry) -> Bool {
        switch (lhs, rhs) {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        let arguments = arguments as! SpaceGramQwenAssistantArguments
        switch self {
        ...
        case let .message(_, section, message, streaming):
            ...
            if message.role == .assistant && !streaming {
                return ItemListTextItem(presentationData: presentationData, text: .custom(context: arguments.context, string: spaceGramRenderAIResponse("\(title): \(message.content)", color: presentationData.theme.list.freeTextColor, linkColor: presentationData.theme.list.itemAccentColor)), sectionId: section, linkAction: { action in
        ...
        case let .newConversation(_, section):
            return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: ngI18n("SpaceGram.AI.NewConversation", presentationData.strings.baseLanguageCode), kind: .generic, alignment: .natural, sectionId: section, style: .blocks, action: arguments.newConversation)
        ...
        case let .deleteConversation(_, section):
            return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: ngI18n("SpaceGram.AI.DeleteConversation", presentationData.strings.baseLanguageCode), kind: .generic, alignment: .natural, sectionId: section, style: .blocks, action: arguments.deleteConversation)
        ...
        case let .search(_, section, value):
            return ItemListSingleLineInputItem(presentationData: presentationData, systemStyle: .glass, title: NSAttributedString(string: ngI18n("SpaceGram.History.Search", presentationData.strings.baseLanguageCode), textColor: presentationData.theme.list.itemPrimaryTextColor), text: value, placeholder: ngI18n("SpaceGram.AI.SearchPlaceholder", presentationData.strings.baseLanguageCode), type: .regular(capitalization: false, autocorrection: false), clearType: .onFocus, sectionId: section, textUpdated: arguments.updateSearch, action: {})
        case let .rename(_, section):
            return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: ngI18n("SpaceGram.AI.Rename", presentationData.strings.baseLanguageCode), kind: .generic, alignment: .natural, sectionId: section, style: .blocks, action: arguments.toggleRename)
        case let .renameInput(_, section, value):
            return ItemListSingleLineInputItem(presentationData: presentationData, systemStyle: .glass, title: NSAttributedString(string: ngI18n("SpaceGram.AI.Title", presentationData.strings.baseLanguageCode), textColor: presentationData.theme.list.itemPrimaryTextColor), text: value, placeholder: ngI18n("SpaceGram.AI.Title", presentationData.strings.baseLanguageCode), type: .regular(capitalization: true, autocorrection: true), clearType: .onFocus, sectionId: section, textUpdated: arguments.updateRename, action: {})
        case let .saveRename(_, section):
            return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: ngI18n("SpaceGram.AI.SaveTitle", presentationData.strings.baseLanguageCode), kind: .generic, alignment: .natural, sectionId: section, style: .blocks, action: arguments.saveRename)
        case let .messageActions(_, section, id):
            return ItemListActionItem(presentationData: presentationData, systemStyle: .glass, title: ngI18n("SpaceGram.AI.MessageActions", presentationData.strings.baseLanguageCode), kind: .generic, alignment: .natural, sectionId: section, style: .blocks, action: { arguments.messageActions(id) })
        }
    }
}

public func spaceGramQwenAssistantController(context: AccountContext, initialText: String = "") -> ViewController {
    let store = SpaceGramConversationStore(mediaBoxPath: context.account.postbox.mediaBox.basePath)
    let updatePromise = ValuePromise<Int32>(0, ignoreRepeated: false)
    var updateValue: Int32 = 0
    var input = initialText
    var messages: [SpaceGramAIMessage] = []
    var current = SpaceGramAIConversation()
    var conversations: [SpaceGramAIConversation] = []
    ...
    var messageLimit = 50
    var isSending = false
    var streamingTask: SpaceGramAIStreamingTask?
    ...
    let startGeneration: (String?, Int?) -> Void = { newText, userIndex in
        ...
        guard SpaceGramSettings.shared.spaceGramEnabled else {
            showError(ngI18n("SpaceGram.Disabled", context.sharedContext.currentPresentationData.with { $0 }.strings.baseLanguageCode))
            return
        }
        if let newText, newText.isEmpty || newText.count > SpaceGramConversationStore.maxContextCharacters {
            showError("This message exceeds the conversation context limit or is empty.")
            return
        }
        let apiKey: String
        do {
            guard let storedKey = try SpaceGramAIKeychain.loadQwenAPIKey(accountId: context.account.id.int64), !storedKey.isEmpty else {
                showError("Configure Qwen Provider in SpaceGram Settings → AI before sending a message.")
                return
            }
            apiKey = storedKey
        } catch {
            showError("Unable to access the Qwen API key. Configure Qwen Provider in SpaceGram Settings → AI.")
            return
        }
        let model = SpaceGramSettings.shared.qwenModel.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !model.isEmpty else {
            showError("Configure a Qwen model in SpaceGram Settings → AI before sending a message.")
            return
        }
        ...
        if let userIndex {
            ...
            sourceMessages.append(SpaceGramAIMessage(role: .user, content: newText))
        } else { return }
        var requestConversation = current
        requestConversation.messages = sourceMessages
        let requestMessages = SpaceGramConversationStore.requestContext(requestConversation, characterBudget: SpaceGramSettings.shared.aiContextCharacters)
        ...
        if let newText {
            messages.append(SpaceGramAIMessage(role: .user, content: newText))
            input = ""
        }
        messages.append(SpaceGramAIMessage(role: .assistant, content: ""))
        current.model = model
        saveConversation()
        isSending = true
        streamingGeneration += 1
        ...
        refresh()
        let provider = SpaceGramQwenProvider(apiKey: apiKey)
        streamingTask = provider.streamText(model: model, messages: requestMessages, onUpdate: { text in
            Queue.mainQueue().async {
                ...
                messages[messages.count - 1] = SpaceGramAIMessage(role: .assistant, content: previous.content + text, id: previous.id, attachments: previous.attachments)
                refresh()
            }
            ...
            Queue.mainQueue().async {
                ...
                case let .failure(error):
                    ...
                    let message: String
                    switch error {
                    case .disabled: message = ngI18n("SpaceGram.Disabled", context.sharedContext.currentPresentationData.with { $0 }.strings.baseLanguageCode)
    ...
    let arguments = SpaceGramQwenAssistantArguments(context: context, updateInput: { value in
        input = value
    }, send: {
        ...
        stop()
        current = SpaceGramAIConversation()
        messages = []
        input = ""
        ...
        var actions: [TextAlertAction] = [
            TextAlertAction(type: .genericAction, title: ngI18n("SpaceGram.AI.Copy", data.strings.baseLanguageCode), action: { UIPasteboard.general.string = selected.content }),
            TextAlertAction(type: .genericAction, title: ngI18n("SpaceGram.AI.Share", data.strings.baseLanguageCode), action: {
        ...
        if selected.role == .user, index == messages.lastIndex(where: { $0.role == .user }) {
            actions.append(TextAlertAction(type: .genericAction, title: ngI18n("SpaceGram.AI.Retry", data.strings.baseLanguageCode), action: { startGeneration(nil, index) }))
        }
        if selected.role == .assistant, index == messages.count - 1,
           let userIndex = messages.lastIndex(where: { $0.role == .user }) {
            actions.append(TextAlertAction(type: .genericAction, title: ngI18n("SpaceGram.AI.Regenerate", data.strings.baseLanguageCode), action: { startGeneration(nil, userIndex) }))
        }
        actions.append(TextAlertAction(type: .destructiveAction, title: ngI18n("SpaceGram.AI.DeleteMessage", data.strings.baseLanguageCode), action: {
        ...
        controller?.present(textAlertController(context: context, title: ngI18n("SpaceGram.AI.MessageActions", data.strings.baseLanguageCode), text: String(selected.content.prefix(120)), actions: actions), in: .window(.root))
    })
    ...
    |> map { presentationData, _, enabled -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        var entries: [SpaceGramQwenAssistantEntry] = [.header(0, 0, current.messages.isEmpty ? ngI18n("SpaceGram.AI.NewConversation", presentationData.strings.baseLanguageCode) : current.title)]
        var stableId: Int32 = 1
        entries.append(.conversations(stableId, 0, ngI18n(showConversations ? "SpaceGram.AI.HideConversations" : "SpaceGram.AI.Conversations", presentationData.strings.baseLanguageCode)))
        stableId += 1
        ...
        if messages.count > messageLimit {
            entries.append(.showEarlier(stableId, 0, ngI18n("SpaceGram.AI.ShowEarlier", presentationData.strings.baseLanguageCode) + " (\(messages.count - messageLimit))"))
            stableId += 1
        }
        ...
        if !messages.isEmpty {
            ...
            let request = SpaceGramConversationStore.requestContext(contextConversation, characterBudget: SpaceGramSettings.shared.aiContextCharacters)
            let count = request.filter { $0.role != .system }.count
            let characters = request.reduce(0) { $0 + $1.content.count }
            // Deliberately approximate, not a tokenizer or billing estimate.
            let tokens = (characters + 3) / 4
            entries.append(.contextInfo(stableId, 0, String(format: ngI18n("SpaceGram.AI.ContextUsage", presentationData.strings.baseLanguageCode), count, characters, tokens)))
            stableId += 1
        }
    ...
    store.list { result in
        Queue.mainQueue().async {
            ...
            case let .failure(error):
                showError("Unable to load saved Qwen conversations. " + error.localizedDescription)
            }
