...
private let spaceGramTranslatorLanguages = ["English", "Russian", "Chinese", "Spanish", "German", "French", "Japanese", "Korean"]
...
private enum SpaceGramTranslatorEntry: ItemListNodeEntry {
    ...
    static func == (lhs: SpaceGramTranslatorEntry, rhs: SpaceGramTranslatorEntry) -> Bool {
        switch (lhs, rhs) {
...
public func spaceGramTranslatorController(context: AccountContext, initialText: String = "") -> ViewController {
    ...
    var targetLanguage = spaceGramTranslatorLanguages[0]
    var result: String?
    ...
    let arguments = SpaceGramTranslatorArguments(updateInput: { value in
        input = value
    }, selectTargetLanguage: {
        guard !isTranslating else {
            return
        }
        let actions = spaceGramTranslatorLanguages.map { language in
        ...
        do {
            guard let storedKey = try SpaceGramAIKeychain.loadQwenAPIKey(accountId: context.account.id.int64), !storedKey.isEmpty else {
                showError("Configure Qwen Provider in SpaceGram Settings → AI before translating.")
                return
            }
            apiKey = storedKey
        } catch {
        ...
        guard !model.isEmpty else {
            showError("Configure a Qwen model in SpaceGram Settings → AI before translating.")
            return
        }
        ...
        refresh()
        let messages = [
            SpaceGramAIMessage(role: .system, content: "You are a translation engine. Translate accurately into the requested target language. Preserve meaning, formatting, names, URLs, and important details. Return only the translated text unless clarification is necessary."),
            SpaceGramAIMessage(role: .user, content: "Target language: \(targetLanguage)\n\nText:\n\(text)"),
        ]
        SpaceGramQwenProvider(apiKey: apiKey).generateText(model: model, messages: messages) { requestResult in
            Queue.mainQueue().async {
                ...
                case let .failure(error):
                    ...
                    switch error {
                    case .disabled:
                        message = ngI18n("SpaceGram.Disabled", context.sharedContext.currentPresentationData.with { $0 }.strings.baseLanguageCode)
                    case .invalidRequest:
