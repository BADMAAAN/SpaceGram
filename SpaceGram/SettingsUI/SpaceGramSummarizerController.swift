...
private final class SpaceGramSummarizerArguments {
...
private enum SpaceGramSummarizerEntry: ItemListNodeEntry {
    ...
    static func == (lhs: SpaceGramSummarizerEntry, rhs: SpaceGramSummarizerEntry) -> Bool {
        switch (lhs, rhs) {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        let arguments = arguments as! SpaceGramSummarizerArguments
        switch self {
...
public func spaceGramSummarizerController(context: AccountContext, initialText: String = "") -> ViewController {
    ...
    let arguments = SpaceGramSummarizerArguments(updateInput: { value in
        input = value
    }, summarize: {
        ...
        do {
            guard let storedKey = try SpaceGramAIKeychain.loadQwenAPIKey(accountId: context.account.id.int64), !storedKey.isEmpty else {
                showError("Configure Qwen Provider in SpaceGram Settings → AI before summarizing.")
                return
            }
            apiKey = storedKey
        } catch {
        ...
        guard !model.isEmpty else {
            showError("Configure a Qwen model in SpaceGram Settings → AI before summarizing.")
            return
        }
        ...
        refresh()
        let messages = [
            SpaceGramAIMessage(role: .system, content: "You summarize text accurately and concisely. Preserve important facts and do not invent information."),
            SpaceGramAIMessage(role: .user, content: text),
        ]
    ...
    |> map { presentationData, _, enabled -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        var entries: [SpaceGramSummarizerEntry] = [
            .header(0, 0, "Text"),
