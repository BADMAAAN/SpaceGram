...
public struct SpaceGramPrivacyPolicy: Codable, Equatable {
    public static let currentVersion = 1
    public static let `default` = SpaceGramPrivacyPolicy()
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard version == Self.currentVersion else {
            throw SpaceGramPrivacyPolicyError.unsupportedVersion
        }
    ...
    public func notificationPresentation(title: String?, subtitle: String?, body: String?, appLocked: Bool) -> SpaceGramNotificationPrivacyPresentation {
        let shouldApply = !self.notificationPrivacyWhenAppLocked || appLocked
        guard shouldApply else {
            return SpaceGramNotificationPrivacyPresentation(title: title, subtitle: subtitle, body: body, allowsRichBody: true, allowsSender: true, allowsAttachments: true)
        }
        let hideIdentity = self.hideNotificationSender || self.hideNotificationChatTitle
        if self.useGenericNotificationText {
            return SpaceGramNotificationPrivacyPresentation(title: "SpaceGram", subtitle: nil, body: "New notification", allowsRichBody: false, allowsSender: false, allowsAttachments: false)
        }
        return SpaceGramNotificationPrivacyPresentation(
            title: hideIdentity ? "SpaceGram" : title,
...
public struct SpaceGramNotificationPrivacyPresentation: Equatable {
...
public enum SpaceGramEmergencyAction: String, Codable, Equatable {
    // Groundwork only. No destructive or remote action is implemented.
    case lockApplication
}

public enum SpaceGramPrivacyPolicyError: Error {
    case unavailable
    case invalidData
    case unsupportedVersion
}

public enum SpaceGramPrivacyPolicyStore {
    // Compatibility contract shared with existing Notification Service policy
    // mirrors. Kept intentionally; reading settings never resets user choices.
    ...
    public static func loadAccountPolicy(mediaBoxPath: String) -> SpaceGramPrivacyPolicy {
        let url = accountPolicyURL(mediaBoxPath: mediaBoxPath)
        guard let values = try? url.resourceValues(forKeys: [.isRegularFileKey, .isSymbolicLinkKey, .fileSizeKey]),
              values.isRegularFile == true, values.isSymbolicLink != true,
              (values.fileSize ?? Int.max) <= 16 * 1024,
              let data = try? Data(contentsOf: url),
              let policy = try? JSONDecoder().decode(SpaceGramPrivacyPolicy.self, from: data) else {
            return .default
        }
        return policy
    }

    public static func effectiveAccountPolicy(mediaBoxPath: String) -> SpaceGramPrivacyPolicy {
        guard SpaceGramSettings.shared.spaceGramEnabled else { return .default }
        return loadAccountPolicy(mediaBoxPath: mediaBoxPath)
    }

    @discardableResult
    public static func saveAccountPolicy(_ policy: SpaceGramPrivacyPolicy, accountId: Int64, mediaBoxPath: String, baseBundleId: String?) -> Bool {
        guard policy.version == SpaceGramPrivacyPolicy.currentVersion,
        ...
        do {
            ...
            guard FileManager.default.fileExists(atPath: parent.path, isDirectory: &isDirectory), isDirectory.boolValue else {
                throw SpaceGramPrivacyPolicyError.unavailable
            }
    ...
    public static func loadNotificationPolicy(accountId: Int64, baseBundleId: String) -> SpaceGramPrivacyPolicy {
        guard SpaceGramSettings.enabledForExtensions(defaults: notificationDefaults(baseBundleId: baseBundleId)) else { return .default }
