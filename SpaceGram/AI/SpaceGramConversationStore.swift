...
public struct SpaceGramAIConversation: Codable, Equatable {
    ...
    public var messages: [SpaceGramAIMessage]

    public init(id: String = UUID().uuidString.lowercased(), title: String = "New conversation", titleIsCustom: Bool = false, model: String = "", createdAt: Date = Date(), updatedAt: Date = Date(), systemPrompt: String? = "You are Qwen, a helpful assistant. Be accurate and concise.", metadata: [String: String]? = nil, messages: [SpaceGramAIMessage] = []) {
        self.version = 2
        self.id = id
        self.title = title
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard storedVersion == 1 || storedVersion == 2 else { throw SpaceGramConversationStoreError.invalidData }
        self.version = 2
        ...
        self.messages = try values.decode([SpaceGramAIMessage].self, forKey: .messages)
    }
}

public enum SpaceGramConversationStoreError: Error {
    case unavailable
    case invalidData
    case capacity
}
...
public final class SpaceGramConversationStore {
    public static let maxContextCharacters = 24000
    private static let queue = DispatchQueue(label: "SpaceGram.Conversations", qos: .utility)
    ...
    public init(mediaBoxPath: String) {
        self.root = URL(fileURLWithPath: mediaBoxPath).deletingLastPathComponent().appendingPathComponent("spacegram-conversations-v1", isDirectory: true)
    }

    public func list(completion: @escaping (Result<[SpaceGramAIConversation], Error>) -> Void) {
        Self.queue.async {
            do {
                ...
                var result: [SpaceGramAIConversation] = []
                for file in files where file.pathExtension == "json" && UUID(uuidString: file.deletingPathExtension().lastPathComponent) != nil {
                    guard let values = try? file.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey, .isSymbolicLinkKey]),
                          ...
                          let conversation = try? JSONDecoder().decode(SpaceGramAIConversation.self, from: Data(contentsOf: file)),
                          conversation.version == 2,
                          conversation.id == file.deletingPathExtension().lastPathComponent else {
                        NSLog("SpaceGramConversations: one saved conversation is unreadable")
                        continue
                    }
    ...
    public func save(_ conversation: SpaceGramAIConversation, completion: @escaping (Result<Void, Error>) -> Void) {
        Self.queue.async {
            do {
                guard conversation.version == 2, UUID(uuidString: conversation.id) != nil else { throw SpaceGramConversationStoreError.invalidData }
                try self.prepareRoot()
                ...
                if !FileManager.default.fileExists(atPath: file.path) {
                    ...
                    guard count < Self.maxConversations else { throw SpaceGramConversationStoreError.capacity }
                }
                let data = try JSONEncoder().encode(conversation)
                guard data.count <= Self.maxConversationBytes else { throw SpaceGramConversationStoreError.capacity }
    ...
    public func remove(id: String, completion: @escaping (Result<Void, Error>) -> Void) {
        Self.queue.async {
            do {
                guard UUID(uuidString: id) != nil else { throw SpaceGramConversationStoreError.invalidData }
                try self.prepareRoot()
        ...
        Self.queue.async {
            do {
                ...
                guard FileManager.default.fileExists(atPath: parent.path, isDirectory: &isDirectory), isDirectory.boolValue else {
                    throw SpaceGramConversationStoreError.unavailable
                }
    ...
    public static func requestContext(_ messages: [SpaceGramAIMessage], characterBudget: Int = SpaceGramConversationStore.maxContextCharacters, messageLimit: Int = 64) -> [SpaceGramAIMessage] {
    ...
    public static func requestContext(_ conversation: SpaceGramAIConversation, characterBudget: Int = SpaceGramConversationStore.maxContextCharacters, messageLimit: Int = 64) -> [SpaceGramAIMessage] {
        var messages = conversation.messages
        if let prompt = conversation.systemPrompt, !prompt.isEmpty {
            messages.insert(SpaceGramAIMessage(role: .system, content: prompt), at: 0)
        }
        return requestContext(messages, characterBudget: characterBudget, messageLimit: messageLimit)
    }

    private func prepareRoot() throws {
        try SpaceGramMigrationCoordinator.migrateDirectory(to: self.root, legacyName: "qwengram-conversations-v1")
        let parent = self.root.deletingLastPathComponent()
        var isDirectory: ObjCBool = false
        guard FileManager.default.fileExists(atPath: parent.path, isDirectory: &isDirectory), isDirectory.boolValue else { throw SpaceGramConversationStoreError.unavailable }
        if FileManager.default.fileExists(atPath: self.root.path) {
            let state = try self.root.resourceValues(forKeys: [.isDirectoryKey, .isSymbolicLinkKey])
            guard state.isDirectory == true, state.isSymbolicLink != true else { throw SpaceGramConversationStoreError.unavailable }
        } else {
