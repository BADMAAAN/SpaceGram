...
public struct SpaceGramAIAttachment: Codable, Equatable {
...
public struct SpaceGramAIMessage: Codable, Equatable {
    ...
    public let attachments: [SpaceGramAIAttachment]

    public init(role: Role, content: String, id: String = UUID().uuidString.lowercased(), attachments: [SpaceGramAIAttachment] = []) {
        self.role = role
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.attachments = try values.decodeIfPresent([SpaceGramAIAttachment].self, forKey: .attachments) ?? []
    }
