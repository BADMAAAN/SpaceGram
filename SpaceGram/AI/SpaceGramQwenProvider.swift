...
public final class SpaceGramQwenProvider: NSObject, SpaceGramAIProvider, SpaceGramAIStreamingProvider {
    ...
    public init(apiKey: String, endpoint: URL = SpaceGramQwenProvider.defaultEndpoint, session: URLSession = .shared) {
        self.apiKey = apiKey
        self.endpoint = endpoint
        self.session = session
    }

    public func generateText(model: String, messages: [SpaceGramAIMessage], completion: @escaping (Result<String, SpaceGramAIError>) -> Void) {
        ...
        let gate = SpaceGramAIRequestGate()
    ...
    @discardableResult
    public func streamText(model: String, messages: [SpaceGramAIMessage], onUpdate: @escaping (String) -> Void, completion: @escaping (Result<Void, SpaceGramAIError>) -> Void) -> SpaceGramAIStreamingTask? {
        guard SpaceGramSettings.shared.spaceGramEnabled else {
            completion(.failure(.disabled))
            return nil
        }
    ...
    private struct ProviderMessage: Encodable {
        let role: SpaceGramAIMessage.Role
        let content: String
    }
...
private final class StreamDelegate: NSObject, URLSessionDataDelegate, SpaceGramAIStreamingTask {
    private let onUpdate: (String) -> Void
    private let completion: (Result<Void, SpaceGramAIError>) -> Void
    private let stateQueue = DispatchQueue(label: "SpaceGram.StreamDelegate")
    ...
    private var gate: SpaceGramAIRequestGate?

    init(onUpdate: @escaping (String) -> Void, completion: @escaping (Result<Void, SpaceGramAIError>) -> Void) {
    ...
    private func finish(_ result: Result<Void, SpaceGramAIError>) {
        ...
        let result: Result<Void, SpaceGramAIError> = self.gate?.wasDisabled == true ? .failure(.disabled) : result
        self.gate?.finish()
        self.gate = nil
        self.task = nil
        self.session = nil
